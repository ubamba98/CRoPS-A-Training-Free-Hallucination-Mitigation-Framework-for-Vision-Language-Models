from methods.sid_method import patch_everything
patch_everything()
...
def run_pope_benchmark(model, processor, args):
    experiment_name = os.path.join("experiments", "--".join(args.model_name.split("/")), "PoPE", args.pope_type, "SID",args.experiment_name)
...
def run_mme_benchmark(model, processor, args):
    experiment_name = os.path.join("experiments", "--".join(args.model_name.split("/")), "SID", "MME")
    os.makedirs(experiment_name, exist_ok=True)
...
def run_mathvista_benchmark(model, processor, args):
    experiment_name = os.path.join("experiments", "--".join(args.model_name.split("/")), "SID", "MathVista")
    os.makedirs(experiment_name, exist_ok=True)
...
def run_chair_benchmark(model, processor, args):
    experiment_name = os.path.join("experiments", "--".join(args.model_name.split("/")), "SID", args.experiment_name)
    os.makedirs(experiment_name, exist_ok=True)
    ...
    with distributed_state.split_between_processes(test_dataset) as process_test_dataset:
        ...
        for test_image in tqdm(process_test_dataset, desc=f"Running Chair Benchmark. Process: {distributed_state.process_index}"):
            ...
            # print(tokens)
            # print(image_tokens[0])
            # print(image_tokens[-1])
            output_text = processor.decode(output_ids[0][len(inputs["input_ids"][0]):], skip_special_tokens=True)            
