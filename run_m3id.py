from methods.m3id_method import patch_everything
patch_everything()
...
def run_mmmu_benchmark(model,processor,args):
    experiment_name = os.path.join("experiments", "--".join(args.model_name.split("/")), "MMMU", "M3ID",args.experiment_name)
...
def run_chair_benchmark(model, processor, args):
    experiment_name = os.path.join("experiments", "--".join(args.model_name.split("/")), "M3ID", args.experiment_name)
    os.makedirs(experiment_name, exist_ok=True)
